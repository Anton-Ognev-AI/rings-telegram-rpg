...
- `docs/superpowers/specs/2026-07-15-phase-04-onboarding-starter-owner-smoke-design.md`: Supplemental Phase 4A local onboarding/starter-build and Phase 4T private owner-smoke design (Status: council-ready draft; awaiting written owner review)
...
- Phase 4 supplemental design now splits local Phase 4A from private remote Phase 4T, fixes migration numbering at `014+`, defines the two-run state machine, teacher rescue, stat/item/ring starter build, useful menu and owner-only no-cron smoke topology. Self-review and five-advisor council returned `APPROVE_WITH_SMALL_CHANGES`; the corrected written spec still awaits owner review before planning or implementation.
