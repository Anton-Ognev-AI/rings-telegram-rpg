...
- Current Phase: Phase 4A Gates 4.2 and 4.3 are implementation-complete but remain `in_progress` under one local `verification_hold`. Source verification is green; the complete local Docker/database verifier and its required repeat are blocked by execution-environment access. Phase 4T remote actions, Parallel Track G and external validation remain gated and have not started.
...
- `docs/checkpoints/2026-07-15-phase-04a-gate-4-2.md`: Canonical build/home projection, guided cards, focused Telegram router and verification evidence (Status: implementation complete; local delivery-fault runtime repeat held by environment; ADR 050)
- `docs/checkpoints/2026-07-15-phase-04a-gate-4-3.md`: Two-day accept/discard E2E source, terminal/snapshot proof, 72-cell strict balance matrix, local-only verifier and council evidence (Status: implementation complete; baseline fire debt blocks Phase 4T and local Docker runtime repeat is held by environment; ADRs 051–052)
...
- Gate 4.2 implementation now routes the playable onboarding through one strict `player_home_v1` projection, server-derived `start_run_v3`, message-bound `pa_` actions and run/state-derived `request_run_render_v2`. Guided stat/item/ring/mastery cards edit one canonical Telegram message; pending decisions and rejected actions have visible recovery paths. Evidence includes 159 unit + 3 property, 16 focused integration, 2 concurrency and 325 pgTAP assertions, clean upgrade/reconcile/lint/checksum checks, and focused Phase 3 E2E regressions. Council verdict is `APPROVE_WITH_VERIFICATION_HOLD`: the corrected delivery-fault test type-checks, but its runtime repeat and the final full Phase 3 verifier are still required because the environment can no longer start/connect to local Supabase (ADR 050).
- Gate 4.3 implementation adds production-shaped two-day Telegram accept/discard flows, exact tutorial/ledger/offer/card/snapshot assertions, terminal/expiry/abandon fixtures, all four starter-build snapshots, a deterministic fail-closed 72-cell ring simulation and a 31-step local-only verifier. Source verification is `168 + 3`; the full baseline/martial/arcane matrix has zero pairwise dominance, while the isolated real-baseline diagnostic pins `weapon > fire`, `defense > fire` and `healing > fire`. Synthetic archetypes are test-only, so this result approves the source balance mechanism but does not clear Phase 4T owner-smoke. Locked-hash/fixture-safety preflights are green and CLI telemetry is disabled. Full runtime stopped at local Docker pipe access, so council verdict is `APPROVE_SOURCE_DELTA_WITH_EXISTING_VERIFICATION_HOLD`, not Phase approval (ADRs 051–052).
- Next safe step: when local Docker permission is available, run `verify:phase4a` twice from clean resets, resolve every mismatch, then write the final Phase 4A checkpoint and lock migration 014. Separately, before Phase 4T owner-smoke, approve and verify real content rotation or a focused balance amendment that removes the baseline fire trap without weakening the pairwise gate. The 5–10-person external core-loop gate remains deferred—not passed or cancelled—until a playable locally verified Telegram build exists. Migrations 001–013 and Phase 1 resolver/config/golden files remain locked; do not link Supabase, deploy, register Telegram webhooks, load real secrets, or invite testers without a separate remote gate.
