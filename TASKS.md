...
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 `approved` locally; Gate 4T.1 is the only active task, staging-only remote authorization is granted, and P4T-05 is paused at the owner-operated Edge secret-storage checkpoint; ADRs 046–067)
  ...
  - [ ] P4T-05: Provision isolated app/recovery staging plus a disposable no-webhook restore target, apply app migrations 001–016 and recovery 001–002, pause for owner-operated secret storage without reading `.env`, deploy internal functions before the owner-only webhook, and prove non-owner rejection (`in_progress`: app/recovery staging are `ACTIVE_HEALTHY`; recovery 001–002 pass the table/service-role/anon proof; app remote history matches 001–016 and a repeat dry-run is up to date; pre-seed state has no active base config, tutorial not enabled and zero Telegram identities; `Instagram` remains `INACTIVE`; restore-target creation is deferred to the deletion drill; owner-operated Edge storage is authoritatively verified as `7/7` required names in app staging and `0` in recovery as expected; secret values and the outside-repository file were not read; internal-function deploy is next; the runbook now distinguishes file creation from remote installation and verifies current/legacy CLI JSON shapes)
