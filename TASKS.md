...
## Active Task
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 `approved` locally; Gate 4T.1 is the only active task, staging-only remote authorization is granted, and P4T-05 is paused at owner-local webhook registration/proof; ADRs 046–067)
  ...
  - [ ] P4T-05: Provision isolated app/recovery staging plus a disposable no-webhook restore target, apply app migrations 001–016 and recovery 001–002, pause for owner-operated secret storage without reading `.env`, deploy internal functions before the owner-only webhook, and prove non-owner rejection (`in_progress`: app/recovery staging are `ACTIVE_HEALTHY`; recovery 001–002 pass the table/service-role/anon proof; app remote history matches 001–016 and a repeat dry-run is up to date; the repeated offline preflight reports `ready`, 9 checks, 18 migrations and 270 tracked files; owner-operated Edge storage is verified as `7/7` required names in app staging and `0` in recovery as expected without reading values; `day-publish-reset` and `outbox-worker` are deployed and each returns `401` both without JWT and with valid anon JWT but no internal secret; `tg-webhook` is deployed separately and returns `401` without its secret; a link drift to recovery staging was caught fail-closed and the worktree was restored to app staging; tracked legacy Supabase JWT material is now rejected and the full source gate passes; Telegram webhook registration is still absent; the pre-registration Telegram identity count is `0`; `Instagram` remains `INACTIVE`; restore-target creation is deferred to the deletion drill; owner-local `setWebhook` plus synthetic non-owner `403` is next)
  - [ ] P4T-06: Run the real three-Kyiv-cycle private Telegram flow, record technical and product evidence, close deletion/recovery/reconciliation checks, remove or explicitly retain staging, and write the Gate 4T.1 checkpoint
