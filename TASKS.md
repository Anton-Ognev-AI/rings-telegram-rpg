...
## Active Task
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 `approved` locally; Gate 4T.1 is the only active task, staging-only remote authorization is granted, and P4T-05 is paused at the owner-operated recovery-project database-password checkpoint; ADRs 046–067)
  ...
  - [ ] P4T-05: Provision isolated app/recovery staging plus a disposable no-webhook restore target, apply app migrations 001–016 and recovery 001–002, pause for owner-operated secret storage without reading `.env`, deploy internal functions before the owner-only webhook, and prove non-owner rejection (`in_progress`: app staging is `ACTIVE_HEALTHY`, linked only from the isolated worktree, and its remote dry-run lists exactly migrations 001–016; the owner authorized temporary `Instagram` suspension and its status is `INACTIVE`; the prepared recovery-project form awaits the owner's database password)
