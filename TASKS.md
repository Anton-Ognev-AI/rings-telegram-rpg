...
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 and P4T-05 staging ingress are `approved`; Gate 4T.1/P4T-06 is the only active task; migration 017 and the amended functions are deployed to owner-only staging, recovery history is aligned through 002, resilient pending-delivery navigation is deployed, the fresh 34-step local Docker gate is clean, Free-plan restore-slot choreography is documented, and owner acceptance plus deletion/restore closeout remain pending; ADRs 046–074)
  ...
  - [ ] P4T-06F7: Replace developer-facing upgrade effects with player-facing current and projected totals, then validate the proposed HP formula separately.
    ...
    - [ ] P4T-06F7B: Simulate the candidate rule `max HP = vitality × 10` against the approved prototype, then use a forward-only config/mechanics amendment if the resulting survivability curve is accepted; do not spoof the new total in presentation before mechanics change.
  - [ ] P4T-06F8: Revisit the long-term stat XP curve. Preserve increasing marginal cost, compare the approved prototype curve with a mild geometric multiplier, and simulate early-day agency plus multi-year progression before changing economy numbers.
  ...
  - [ ] P4T-06F10: Simulate a larger expedition whose required power rises geometrically, preserving an accessible first 3–4 stages and making deeper progress an achievement. Compare fixed depth, extended depth and a weekly learnable route before any forward-only mechanics/content amendment.
