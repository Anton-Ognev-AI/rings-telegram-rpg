...
## Active Task
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 and P4T-05 staging ingress are `approved`; Gate 4T.1/P4T-06 is the only active task, the owner-only runner is live for the first real Kyiv cycle, and deletion/restore closeout remains pending; ADRs 046–067)
  ...
  - [ ] P4T-06: Run the real three-Kyiv-cycle private Telegram flow, record technical and product evidence, close deletion/recovery/reconciliation checks, remove or explicitly retain staging, and write the Gate 4T.1 checkpoint (`in_progress`: cycle `2026-07-19` is open/playable; a first-owner-start incident was repaired by moving Telegram from the recovery 404 target to app staging and canonicalizing `RECOVERY_SUPABASE_URL`; the full owner-only webhook probe now returns `200`, Telegram pending updates are `0`, the owner identity is bootstrapped, and the no-cron runner remains live for continued play)
