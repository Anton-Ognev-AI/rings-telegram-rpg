...
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 and P4T-05 staging ingress are `approved`; Gate 4T.1/P4T-06 is the only active task; migration 017 and the amended functions are deployed to owner-only staging, recovery history is aligned through 002, resilient pending-delivery navigation is deployed, the fresh 34-step local Docker gate is clean, Free-plan restore-slot choreography is documented, and owner acceptance plus deletion/restore closeout remain pending; ADRs 046–074)
  ...
  - [ ] P4T-06F9: Design resilient Telegram navigation around one current bottom control surface. Historical navigation buttons should resolve to the current canonical state; historical state-changing buttons must fail safely or redirect, never create duplicate mutations or an endless Telegram spinner.
    ...
    - [ ] P4T-06F9B: Add one non-intrusive persistent `Меню` control only if it coexists cleanly with inline quest choices; the menu owns `Повернутися в експедицію`, `Герой`, `Академія`, and `Допомога`. Preserve canonical routing for old inline controls.
