...
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 and P4T-05 staging ingress are `approved`; Gate 4T.1/P4T-06 is the only active task; migration 017 and the amended functions are deployed to owner-only staging, recovery history is aligned through 002, resilient pending-delivery navigation is deployed, the fresh 34-step local Docker gate is clean, Free-plan restore-slot choreography is documented, and owner acceptance plus deletion/restore closeout remain pending; ADRs 046–077)
  ...
  - [x] P4T-06F9: Design resilient Telegram navigation around one current bottom control surface. Historical navigation buttons should resolve to the current canonical state; historical state-changing buttons must fail safely or redirect, never create duplicate mutations or an endless Telegram spinner.
    ...
    - [x] P4T-06F9B: Add one non-intrusive persistent `Меню` control only if it coexists cleanly with inline quest choices; the menu owns `Повернутися в експедицію`, `Герой`, `Академія`, and `Допомога`. Preserve canonical routing for old inline controls (`approved locally`: one final inline row is present on active stages, resolved-next cards and blocked field discoveries; it creates no profile mutation; explicit menu navigation cannot auto-resume the active run; `Повернутися в експедицію` restores the exact run/offer; and the full source gate is green at 223 unit + 3 property. ADR 077; checkpoint `docs/checkpoints/2026-07-23-phase-04t-expedition-menu.md`; staging deployment remains part of P4T-06 owner acceptance).
