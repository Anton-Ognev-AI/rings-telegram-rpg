...
## Active Task
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 and P4T-05 staging ingress are `approved`; Gate 4T.1/P4T-06 is the only active task; migration 017 and the amended functions are deployed to owner-only staging, recovery history is aligned through 002, resilient navigation is locally corrected for a cached `/start`, the fresh 34-step local Docker gate is clean, Free-plan restore-slot choreography is documented, and owner acceptance plus deletion/restore closeout remain pending; ADRs 046–078)
  ...
  - [ ] P4T-06: Run the real three-Kyiv-cycle private Telegram flow, record technical and product evidence, close deletion/recovery/reconciliation checks, remove or explicitly retain staging, and write the Gate 4T.1 checkpoint (`in_progress`: cycle `2026-07-19` reached owner stage 6 and produced the F1–F6 amendment; clarity Phase A, character Phase B, discovery C1 and F9B expedition navigation are deployed to owner-only app staging. App history is exactly 001–017 and recovery history is exactly 001–002. Owner evidence on 2026-07-23 exposed a cached `/start` with no visible reply and a session runner that had exited after one transient worker 500; both bounded corrections are deployed/active after a GREEN 225-unit + 3-property source gate and final 9-check/19-migration/297-file remote preflight. The local coupled-progression checkpoint has also closed F8B/F10B and split the remaining mechanics design into F7B2B without changing runtime truth. Owner Telegram acceptance, an encrypted pre-deletion recovery point and the isolated deletion/restore drill remain pending; checkpoints: `docs/checkpoints/2026-07-22-phase-04t-discovery-staging-deploy.md`, `docs/checkpoints/2026-07-22-phase-04t-recovery-readiness.md`, `docs/checkpoints/2026-07-23-phase-04t-expedition-menu.md`, `docs/checkpoints/2026-07-23-phase-04t-start-response-recovery.md`, `docs/checkpoints/2026-07-23-phase-04t-coupled-progression-matrix.md`)
  ...
  - [x] P4T-06R5: Make the session runner tolerate isolated transient worker faults without hiding sustained failure (`approved staging-only`: one worker 500 is retried after the normal interval, success resets the counter, and the third consecutive failure ends the process with the same generic redacted error; RED→GREEN regression and full source gate pass. The 08:09 session later ended fail-closed at 08:28 after three consecutive rejected worker requests, confirming the stop boundary. A staging-only in-memory secret rotation plus `200/ok` day/worker probes established a fresh runner; dashboard evidence shows repeated two-second worker `200`s from 08:54)
  ...
  - [ ] P4T-06F2: Design and implement between-stage discoveries so each resolved quest may lead to a meaningful bonus/item offer before continuing, while preserving three equipment slots, no inventory, immediate replace/discard and protected reward pacing
    ...
    - [ ] P4T-06F2D: Extend the proven C1 boundary to general post-tutorial rarity/smart-loot/boss-drop pacing only after owner acceptance; a find is eligible only after a fully successful stage (fitting choice and passed stat check), while XP/HP consequences remain independent; do not widen migration 017 retrospectively
  - [ ] P4T-06F3: Design coherent Academy story arcs: a day has a named lesson and escalating practice; later days continue with a connected field assignment (for example, an expedition into the forest) rather than unrelated scenes. Compare a fresh daily route with a weekly learnable route plus daily attempts/variation before choosing the cadence.
  ...
  - [ ] P4T-06F7: Replace developer-facing upgrade effects with player-facing current and projected totals, then validate the proposed HP formula separately.
    - [x] P4T-06F7A: Remove raw internal effect labels (`physical +1`, `vitality +1`) and render structured current → projected totals from the canonical build plus server forecast (`approved and deployed to owner-only staging`; focused 23/23 and source gate 218 unit + 3 property green; ADR 075; checkpoint `docs/checkpoints/2026-07-22-phase-04t-stat-projection.md`)
    - [x] P4T-06F7B1: Prove why `max HP = vitality × 10` cannot ship alone and establish a coupled candidate (`approved for simulation only`: HP-only lets the tutorial starter survive all nine ordinary V1 stages; candidate attrition restores neutral/failure boundaries at stages 6/5; ADR 076)
    - [ ] P4T-06F7B2: Run the candidate through a full resolver/content/build/friend/ring matrix, then implement accepted HP changes through a forward-only versioned mechanics/config amendment; do not spoof the new total in presentation before mechanics change.
      - [x] P4T-06F7B2A: Produce a deterministic coupled matrix over resolver-V1 evidence and candidate build/ring/friend/XP states (`approved local evidence`: 96 current-V1 reports plus 480 candidate cells; 456 supported, 24 explicitly unsupported higher-tier healing cells; canonical SHA `99ed3da0b353821fdf3d76caefeaefc1719dfea5db3e161910effcdef5ad28f5`; 5/5 focused and full source gate 230 unit + 3 property green; no runtime change; ADR 079)
      - [ ] P4T-06F7B2B: Define an explicit support success metric, higher-tier healing output and dual-ring stacking; council-review the forward-only resolver/config V2 amendment before implementation. Preserve V1 party aggregation during design rather than changing it to make the candidate pass.
  - [x] P4T-06F8: Revisit the long-term stat XP curve without weakening the shared XP/ring economy (`simulation evidence complete`; any migration remains coupled to F7B2B and owner telemetry).
    - [x] P4T-06F8A: Compare the prototype quadratic stat curve with a mild geometric candidate (`ceil(20 × 1.20^n)`) and pin early/late evidence (`approved for simulation only`: first ten costs `20,24,29,35,42,50,60,72,86,104`; 30 purchases cost 23,648 XP; 43 XP permits one repeated-specialization upgrade or two first upgrades in different stats; ADR 076)
    - [x] P4T-06F8B: Include ring budgets, realistic earned-XP distributions and horizontal builds in the multi-year matrix before any economy migration or staging deployment (`approved local evidence`: 96 resolver-V1 reports produce 10–60 XP, median 28 and p75 43; one purple ring takes about 7.63/4.97/3.56 years at observed 28/43/60 XP per day; 43 XP buys one focused or two first horizontal stat upgrades; no economy migration approved; ADR 079).
  - [x] P4T-06F9: Design resilient Telegram navigation around one current bottom control surface. Historical navigation buttons should resolve to the current canonical state; historical state-changing buttons must fail safely or redirect, never create duplicate mutations or an endless Telegram spinner.
    - [x] P4T-06F9A: Recover the owner-smoke missing-first-card incident without duplicate mutation (`approved and deployed to owner-only staging`: cached starts route to canonical resume; exact `card_unavailable` sends a safe pending-delivery card; every other rejection remains fail-closed; focused 33/33, real Postgres no-duplicate regression, adjacent 8/8 E2E, source gate and repeated 34-step Docker gate green; ADR 074; checkpoint `docs/checkpoints/2026-07-22-phase-04t-navigation-recovery.md`)
    - [x] P4T-06F9B: Add one non-intrusive persistent `Меню` control only if it coexists cleanly with inline quest choices; the menu owns `Повернутися в експедицію`, `Герой`, `Академія`, and `Допомога`. Preserve canonical routing for old inline controls (`deployed to owner-only staging; acceptance pending`: one final inline row is present on active stages, resolved-next cards and blocked field discoveries; it creates no profile mutation; explicit menu navigation cannot auto-resume the active run; `Повернутися в експедицію` restores the exact run/offer; full source gate is 223 unit + 3 property; remote preflight is 9 checks / 19 migrations / 296 files; worker/webhook are ACTIVE at version 31 with JWT `true/false`. ADR 077; checkpoint `docs/checkpoints/2026-07-23-phase-04t-expedition-menu.md`).
    - [ ] P4T-06F9C: Give `/start` a visible read-only response when the active expedition card is already current (`deployed to owner-only app staging; acceptance pending`: canonical `request_run_render_v2` and run recovery remain unchanged; a clean active run receives the navigation menu immediately, pending tutorial item/ring decisions remain authoritative, and a blocked field discovery remains resumable; RED→GREEN handler regressions, full source gate, final remote preflight and three 401 negative-auth probes pass. Dashboard evidence shows no owner `/start` after deployment—only the deliberate 08:07 wrong-secret probe—so acceptance requires one fresh `/start`; the earlier 07:33 inbound update cannot be replayed. ADR 078).
  - [x] P4T-06F10: Simulate a larger expedition whose required power rises geometrically, preserving an accessible first 3–4 stages and making deeper progress an achievement (`simulation evidence complete`; playable MVP remains ten stages).
    - [x] P4T-06F10A: Add a test-only extensible curve candidate (`threshold = ceil(5 × 1.35^(stage−1))`, coupled neutral/failure attrition) and prove power 13 reaches stage 4 while power 25 reaches stage 6 (`approved for simulation only`; four RED→GREEN tests; ADR 076; checkpoint `docs/checkpoints/2026-07-22-phase-04t-progression-curve-simulation.md`)
    - [x] P4T-06F10B: Compare 10-stage, 12-stage and future extended resolver-V2 matrices with actual rings/items/friends and Telegram session length; keep the playable MVP at 10 stages until that evidence justifies more (`approved local evidence`: no supported candidate gains fitting reward depth beyond stage 10; 12/20 stages add mostly unrewarded survival and increase the run from 11–12 decisions to 13–14/21–22; recommendation remains `keep-10-until-healing-v2-content-and-owner-session-evidence`; ADR 079).
