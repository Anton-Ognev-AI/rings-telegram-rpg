...
- [ ] ID: PHASE-04T: Private Telegram owner-smoke (Gate 4T.0 and P4T-05 staging ingress are `approved`; Gate 4T.1/P4T-06 is the only active task; migration 017 and the amended functions are deployed to owner-only staging, recovery history is aligned through 002, resilient navigation is locally corrected for a cached `/start`, the fresh 34-step local Docker gate is clean, Free-plan restore-slot choreography is documented, and owner acceptance plus deletion/restore closeout remain pending; ADRs 046–078)
  ...
  - [ ] P4T-06F8: Revisit the long-term stat XP curve without weakening the shared XP/ring economy.
    ...
    - [ ] P4T-06F8B: Include ring budgets, realistic earned-XP distributions and horizontal builds in the multi-year matrix before any economy migration or staging deployment.
  - [x] P4T-06F9: Design resilient Telegram navigation around one current bottom control surface. Historical navigation buttons should resolve to the current canonical state; historical state-changing buttons must fail safely or redirect, never create duplicate mutations or an endless Telegram spinner.
    ...
    - [ ] P4T-06F9C: Give `/start` a visible read-only response when the active expedition card is already current (`deployed to owner-only app staging; acceptance pending`: canonical `request_run_render_v2` and run recovery remain unchanged; a clean active run receives the navigation menu immediately, pending tutorial item/ring decisions remain authoritative, and a blocked field discovery remains resumable; RED→GREEN handler regressions, full source gate, final remote preflight and three 401 negative-auth probes pass. ADR 078).
  - [ ] P4T-06F10: Simulate a larger expedition whose required power rises geometrically, preserving an accessible first 3–4 stages and making deeper progress an achievement.
    ...
    - [ ] P4T-06F10B: Compare 10-stage, 12-stage and future extended resolver-V2 matrices with actual rings/items/friends and Telegram session length; keep the playable MVP at 10 stages until that evidence justifies more.
